...
const passageDiplomeBoulanger: (perso: Perso) => Promise<string> = (perso: Perso) => {
    ...
    const resTestMetier:ResultatTest = testMetier(perso, MetiersEnum.apprenti_boulanger, 20);
    ...
    if (resTestMetier.reussi) {
        ...
        texte += commencerCarriere(perso, MetiersEnum.boulanger, '', false);
    } else {
    ...
    evts: [
        {
            ...
            description: async (perso: Perso): Promise<string> => {
                ...
                if (resTestDex.reussi) {
                    texte += commencerCarriere(perso, MetiersEnum.apprenti_boulanger, '', true);
            ...
            conditions: (perso: Perso): boolean =>
                ...
                && compatibiliteCarriere(perso, metiersObjs[MetiersEnum.apprenti_boulanger]) >= SEUIL_COMPATIBILITE_METIER
                && getAge(perso) >= 14,
        },
        {
            id: "evts_boulanger2",
            description: async (perso: Perso): Promise<string> => {
                let texte: string = "";
                const resTestMetier:ResultatTest = testMetier(perso, MetiersEnum.boulanger, 0);
                texte += resTestMetier.resume;
                if (resTestMetier.reussi) {
